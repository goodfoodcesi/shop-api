...
import { shop, shopDocument, shopAccess, shopStatusHistory } from '@/db/schemas/shop.js'
import type { Shop, InsertShop, ShopDocument, ShopAccess, CreateShopDTO } from './model'
import type { ShopStatus } from '@/shared/types/shop.types.js'
...
export async function findShopsByStatus(status?: ShopStatus) {
  if (!status) {
    return db.select().from(shop).orderBy(desc(shop.createdAt));
  }

  return db
    .select()
    .from(shop)
    .where(eq(shop.status, status))
